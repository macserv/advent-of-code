//
//  IfYouGiveASeedAFertilizer.swift
//
//  Created by Matthew Judy on 2024-11-18.
//
...
/// only turned off the water a few days... weeks... oh no." His face sinks
...
/// describes how to convert a **seed number** (the source) to a **soil
/// number** (the destination).  This lets the gardener and his team know
/// which soil to use with which seeds, which water to use with which
...
/// values are `50` and `51`.  With this information, you know that seed number
/// `98` corresponds to soil number `50` and that seed number `99` corresponds
/// to soil number `51`.
///
...
/// through other categories until you can find its corresponding
...
@main
struct IfYouGiveASeedAFertilizer: AsyncParsableCommand
{
...
extension IfYouGiveASeedAFertilizer
{
    mutating func run() async throws
    {
        ...
        for keyPath in Almanac.allKeyPaths
        {
            while let mapping: String = try? await input.next(), (false == mapping.hasSuffix(" map:"))
            {
                print(keyPath)
                print(mapping)
                ...
                let length = values[2]
                almanac[keyPath: keyPath][sourceStart...(sourceStart +  length)] = destinationStart...(destinationStart +  length)
            }
        }
