 ...
 * Finally, stack 3 contains a single crate, P.
 ...
 air conditioning, leather seats, an extra cup holder, and the ability to pick
 ...
 those three moved crates stay in the same order, resulting in this
 new configuration:
 ...
 */
@main
struct SupplyStacks: ParsableCommand
{
    ...
    @Option(help: "CrateMover™ model used to execute the move: 'CM9000' or 'CM9001'.")
    var mode: Mode
}
...
extension SupplyStacks
{
    mutating func run() throws
    {
        ...
        typealias CrateStack = Array<Crate>

        let crateLinePattern = /(?:(?:\[[A-Z]\]| {3}) ?)+/
        ...
        while let inputLine = readLine()
        {
            if let _ = inputLine.wholeMatch(of: crateLinePattern)
            {
                ...
                .forEach
                {
                    ...
                    if ( crateStacks.count < (index + 1) )
                    {
                        crateStacks.append(CrateStack())
                    }

                    guard let crate else { return }

                    crateStacks[index].append(crate)
                }
            ...
            if let moveMatch = inputLine.wholeMatch(of: moveLinePattern)
            {
                ...
                let destination : Int = (Int(moveMatch.3)! - 1)
                let payload     : [Crate] =
                {
                    let payloadItems = crateStacks[source].prefix(moveCount)

                    switch self.mode
                    {
                        case .CM9000: return Array(payloadItems.reversed())
                        case .CM9001: return Array(payloadItems)
                    }
                }()

                crateStacks[source].removeFirst(moveCount)
                crateStacks[destination].insert(contentsOf: payload, at: 0)

                continue
            }
        }

        print(String(crateStacks.compactMap(\.first)))
    }
}
