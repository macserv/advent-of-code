...
//  CampCleanup.swift
//
//  Created by Matthew Judy on 2022-12-04.
//
 ...
 * Within the first pair of Elves,
    * the first Elf was assigned sections 2-4 (sections 2, 3, and 4), while
 ...
 * The Elves in the third pair were each assigned three sections:
    * one got sections 5, 6, and 7, while
 ...
 * the first two pairs (2-4,6-8 and 2-3,4-5) don't overlap, while
...
struct CampCleanup: ParsableCommand
{
    ...
    @Option(help: "Check for 'partial' or 'complete' overlap.")
    var overlapCheck: OverlapCheckMode = .complete
}
...
extension CampCleanup
{
    mutating func run() throws
    {
        ...
        {
            let rangeStrings = inputLine.components(separatedBy:",")
            let ranges = rangeStrings.reduce([ClosedRange<Int>]())
            {
                let rangeComponents = $1.components(separatedBy:"-")
                return ( $0 + [ Int(rangeComponents[0])!...Int(rangeComponents[1])! ] )
            }
            ...
            {
                case .partial where leftRange.overlaps(rightRange):
                    overlappingPairs += 1

                case .complete where ( leftRange.covers(rightRange) || leftRange.isCoveredBy(rightRange) ):
                    overlappingPairs += 1
